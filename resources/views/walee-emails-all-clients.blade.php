...
<head>
    ...
    <title>Walee - Todos los Clientes (en proceso)</title>
    <meta name="description" content="Walee - Todos los Clientes (tabla clientes_en_proceso)">
...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-white transition-colors duration-200 min-h-screen">
    <div class="min-h-screen relative overflow-hidden">
        ...
        <div class="relative max-w-[90rem] mx-auto px-3 py-4 sm:px-4 sm:py-6 lg:px-8">
            @php $pageTitle = 'Todos los Clientes (tabla clientes_en_proceso)'; @endphp
            ...
            <header class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-0 mb-3 sm:mb-4">
                <div>
                    <h1 class="text-lg sm:text-xl font-bold text-slate-900 dark:text-white">
                        Todos los Clientes en Proceso (todos los estados)
                    </h1>
                </div>
            ...
            <div class="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-3 shadow-sm">
                <div class="flex items-center justify-between mb-3">
                    <h2 class="text-sm font-semibold text-slate-900 dark:text-white">
                        Todos los Clientes (clientes_en_proceso)
                            ...
                            <div class="flex items-center gap-2 mb-0.5">
                                ...
                                @if($cliente->estado)
                                    <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold 
                                        {{ $cliente->estado === 'pending' ? 'bg-violet-100 text-violet-700 dark:bg-violet-500/20 dark:text-violet-300' : '' }}
                                        {{ $cliente->estado === 'received' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-300' : '' }}
                                        {{ $cliente->estado !== 'pending' && $cliente->estado !== 'received' ? 'bg-slate-100 text-slate-700 dark:bg-slate-700/50 dark:text-slate-200' : '' }}
                                    ">
