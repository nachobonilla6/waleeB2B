...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-white transition-colors duration-200 min-h-screen">
                    ...
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4">
                        ...
                        <div class="flex items-center gap-2 sm:gap-2 flex-shrink-0 flex-wrap header-actions">
                            ...
                            @else
                                ...
                                <button onclick="sincronizarCitasBD()" class="px-3 py-2 sm:px-4 sm:py-2 rounded-lg bg-amber-500 hover:bg-amber-600 text-white font-semibold text-sm transition-all flex items-center justify-center shadow-md hover:shadow-lg active:scale-95" title="Sincronizar citas y tareas bidireccionalmente con Google Calendar">
                                    <svg class="w-4 h-4 sm:w-5 sm:h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-7 divide-y md:divide-y-0 md:divide-x divide-slate-200 dark:divide-slate-700">
                            ...
                            @for($i = 0; $i < 7; $i++)
                                ...
                                <div class="min-h-[300px] md:min-h-[400px] flex flex-col border-b md:border-b-0 md:border-r border-slate-200 dark:border-slate-700 last:border-b-0">
                                    ...
                                    <div class="p-3 md:p-3 border-b border-slate-200 dark:border-slate-700 {{ $esHoy ? 'bg-violet-50 dark:bg-violet-500/10' : 'bg-slate-50 dark:bg-slate-800/30' }}">
                                        ...
                                        <div class="flex gap-1.5 mt-2">
                                            <button 
                                                ...
                                                class="flex-1 px-2 py-1.5 text-xs font-medium rounded-lg bg-violet-500 hover:bg-violet-600 text-white transition-all flex items-center justify-center gap-1 shadow-sm hover:shadow-md active:scale-95"
                                            ...
                                            <button 
                                                ...
                                                class="flex-1 px-2 py-1.5 text-xs font-medium rounded-lg bg-amber-500 hover:bg-amber-600 text-white transition-all flex items-center justify-center gap-1 shadow-sm hover:shadow-md active:scale-95"
                                                title="Crear tarea a las 9 AM">
                                                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                                </svg>
                                                <span>Tarea</span>
                                    ...
                                    <!-- Eventos del día -->
                                    <div class="flex-1 p-3 md:p-2 relative overflow-y-auto" style="min-height: 300px;">
                                        @if($eventosOrdenados->count() > 0)
                                            @foreach($eventosOrdenados as $evento)
                                                @php
                                                    ...
                                                    // Cada hora = 100% / 24 = 4.1667%
                                                    $topPorcentaje = ($horaInicio / 24) * 100;
                                                    $alturaPorcentaje = ($duracionHoras / 24) * 100;
                                                    
                                                    // Altura mínima de 30px o 2% del contenedor
                                                    $alturaMinima = max(30, ($alturaPorcentaje / 100) * 300);
                                                    
                                                    $hora = $fechaInicio->format('H:i');
                                                    $horaFinStr = $fechaFin->format('H:i');
                                                    $titulo = $evento->titulo ?? 'Sin título';
                                                @endphp
                                                <div class="group absolute left-2 right-2" style="top: {{ $topPorcentaje }}%; height: {{ $alturaPorcentaje }}%; min-height: {{ $alturaMinima }}px;">
                                                    <button 
                                                    ...
                                                    <div class="flex flex-col h-full justify-between">
                                                        <div class="flex items-center justify-between gap-1">
                                                            <span class="text-[10px] font-semibold">{{ $hora }}</span>
                                                            @if($horaFinStr !== $hora)
                                                        ...
                                                        <p class="text-xs font-medium text-slate-700 dark:text-slate-300 line-clamp-2 flex-1">{{ $titulo }}</p>
                                                    </div>
                                                    </button>
                                                    @if(isset($evento->is_tarea) && $evento->is_tarea)
                                                    ...
                                                    @elseif(isset($evento->google_event_id) && $evento->google_event_id)
                                                        <div class="absolute top-1 right-1 flex gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                                            <button 
    ...
    <script>
        ...
        function showNuevaTareaModalConFecha(fecha, hora) {
            const isDarkMode = document.documentElement.classList.contains('dark');
            ...
            Swal.fire({
                ...
                html: `
                    <form id="nuevaTareaForm" class="space-y-3 text-left">
                        ...
                        <div>
                            <label class="block text-sm font-medium ${isDarkMode ? 'text-slate-300' : 'text-slate-700'} mb-1">Fecha y Hora</label>
                            <input type="datetime-local" id="tarea_fecha" value="${fechaHora}" required
                                class="w-full px-3 py-2 text-sm ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-300 text-slate-800'} border rounded-lg focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 focus:outline-none">
                        </div>
                        <div>
                            <label class="block text-sm font-medium ${isDarkMode ? 'text-slate-300' : 'text-slate-700'} mb-1">Color</label>
