                ...
                <div class="rounded-xl bg-violet-50 dark:bg-violet-900/20 border border-violet-200 dark:border-violet-800 p-3 md:p-4">
                    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-3 md:gap-4">
                        <div class="flex items-center gap-2 md:gap-3 min-w-0 flex-1">
                            @if($client)
                                <a href="{{ route('walee.cliente.detalle', $client->id) }}" class="w-10 h-10 md:w-12 md:h-12 rounded-lg overflow-hidden flex-shrink-0 hover:opacity-90 transition-opacity cursor-pointer border-2 border-blue-500/30 dark:border-blue-500/20">
                                    @if($fotoUrl)
                                        <img src="{{ $fotoUrl }}" alt="{{ $client->name }}" class="w-full h-full object-cover">
                                    @else
                                        <div class="w-full h-full bg-gradient-to-br from-blue-500/20 to-violet-500/20 flex items-center justify-center">
                                            <span class="text-lg md:text-xl font-bold text-blue-600 dark:text-blue-400">{{ strtoupper(substr($client->name, 0, 1)) }}</span>
                                        </div>
                                    @endif
                                </a>
                            @else
                                <div class="w-10 h-10 md:w-12 md:h-12 rounded-lg bg-blue-100 dark:bg-blue-900/40 flex items-center justify-center flex-shrink-0">
                                    <svg class="w-5 h-5 md:w-6 md:h-6 text-blue-600 dark:text-blue-400" fill="currentColor" viewBox="0 0 24 24">
        ...
        function showProgramarPublicacionModal(fecha = null, hora = null) {
            ...
            const html = `
                <form id="programar-publicacion-form" class="space-y-2.5 text-left w-full ${isMobile ? 'max-w-full' : ''}">
                        ...
                        <div class="space-y-2">
                            ...
                            <div>
                                ...
                                <div class="flex gap-2">
                                    <textarea name="texto" id="texto_publicacion" rows="3" placeholder="Escribe el texto o genera con AI..." class="flex-1 px-2.5 py-1.5 text-xs ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-300 text-slate-800'} border rounded-lg placeholder-slate-500 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 focus:outline-none transition-all resize-none"></textarea>
                    ...
                    <!-- Fecha y Hora (Facebook por defecto) -->
                    <input type="hidden" name="plataforma_publicacion" value="facebook">
                    <div>
                        <label class="block text-xs font-medium ${isDarkMode ? 'text-slate-300' : 'text-slate-700'} mb-1">Fecha y Hora</label>
                        <input type="datetime-local" name="fecha_publicacion" id="fecha_publicacion" required value="${fechaDefault}" class="w-full px-2.5 py-1.5 text-xs ${isDarkMode ? 'bg-slate-800 border-slate-700 text-white' : 'bg-slate-50 border-slate-300 text-slate-800'} border rounded-lg focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 focus:outline-none transition-all">
                    </div>
                </form>
            `;
        ...
        async function showEventoDetail(eventoId) {
            ...
            try {
                ...
                if (data.evento) {
                    ...
                    let facebookHTML = '';
                    if (evento.plataforma === 'facebook' && evento.facebook_link) {
                        facebookHTML = `
                            <a href="${evento.facebook_link}" target="_blank" class="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-blue-100 dark:bg-blue-900/20 hover:bg-blue-200 dark:hover:bg-blue-900/30 text-blue-600 dark:text-blue-400 border border-blue-300 dark:border-blue-700 transition-all">
