...
class ListPropuestaPersonalizadas extends ListRecords
{
    ...
    {
        return [
            Actions\Action::make('crear_propuesta')
                ->label('➕ Crear Propuesta Personalizada')
                ->icon('heroicon-o-plus')
                ->color('success')
                ->modalHeading('📧 Crear Propuesta Personalizada')
                ->modalWidth('2xl')
                ->form([
                    ...
                    Forms\Components\Actions::make([
                        Forms\Components\Actions\Action::make('llenar_con_ai')
                            ...
                            ->action(function (Set $set, Get $get) {
                                ...
                                $clientName = $get('cliente_nombre') ?: ($client?->name ?? 'el cliente');
                    ...
                    Forms\Components\Toggle::make('enviar_email')
                        ...
                        ->default(true)
                        ->helperText('Si está activado, se enviará el email al cliente además de guardarlo.'),
                ])
                ->action(function (array $data) {
                    try {
                        $client = $data['cliente_id'] ? Client::find($data['cliente_id']) : null;
                        ...
                        PropuestaPersonalizada::create([
                            ...
                            'cliente_nombre' => $data['cliente_nombre'] ?? 'N/A',
                            'email' => $data['email'],
                            'subject' => $data['subject'],
                            'body' => $data['body'],
