...
<head>
    ...
    <title>Walee - Todos los Clientes</title>
    <meta name="description" content="Walee - Todos los Clientes (tabla clientes)">
...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-white transition-colors duration-200 min-h-screen">
    <div class="min-h-screen relative overflow-hidden">
        ...
        <div class="relative max-w-[90rem] mx-auto px-3 py-4 sm:px-4 sm:py-6 lg:px-8">
            @php $pageTitle = 'Todos los Clientes (tabla clientes)'; @endphp
            ...
            <div class="mb-3">
                <div class="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-3 shadow-sm">
                    <div class="flex gap-2 items-center">
                        <form method="GET" action="{{ route('walee.emails.all.clients') }}" class="flex gap-2 flex-1">
                            ...
                            @if($searchQuery ?? '')
                                <a href="{{ route('walee.emails.all.clients') }}" class="px-3 py-2 bg-slate-200 dark:bg-slate-700 hover:bg-slate-300 dark:hover:bg-slate-600 text-slate-900 dark:text-white font-medium rounded-lg transition-all flex items-center gap-1.5 text-xs">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg p-3 shadow-sm">
                <div class="flex items-center justify-between mb-3">
                    <h2 class="text-sm font-semibold text-slate-900 dark:text-white">
                        ...
                        <span class="text-xs font-normal text-slate-500 dark:text-slate-400">
                            ({{ $clientesProceso->total() }})
                        </span>
                    </h2>
                </div>
                
                <div class="space-y-2">
                @forelse($clientesProceso as $cliente)
                    <a href="{{ route('walee.planeador.publicidad', $cliente->id) }}" class="flex items-center gap-2.5 p-2.5 rounded-lg bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 hover:border-blue-400 dark:hover:border-blue-500/30 hover:bg-blue-50/50 dark:hover:bg-blue-500/10 transition-all group">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="flex items-center gap-2 mb-0.5">
                                <p class="font-medium text-sm text-slate-900 dark:text-white truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{{ $cliente->nombre_empresa ?: 'Sin nombre' }}</p>
                            </div>
                            @if($cliente->correo)
                                <p class="text-xs text-slate-600 dark:text-slate-400 truncate">{{ $cliente->correo }}</p>
                            @endif
                            ...
                            @if($cliente->url_sitio)
                                <p class="text-xs text-slate-500 dark:text-slate-500 truncate">{{ $cliente->url_sitio }}</p>
                            @endif
                        ...
                        <div class="flex-shrink-0 flex items-center gap-1.5">
                            <div class="bg-blue-100 dark:bg-blue-500/20 border border-blue-200 dark:border-blue-500/30 rounded-lg px-2 py-1 flex items-center gap-1">
                                ...
                                <span class="text-xs font-semibold text-blue-600 dark:text-blue-400">{{ $cliente->facturas_count ?? 0 }}</span>
                            </div>
                        </div>
                    </a>
                @empty
                ...
                <!-- Pagination -->
                @if($clientesProceso->hasPages())
                    <div class="mt-4 flex justify-center gap-2">
                        @if($clientesProceso->onFirstPage())
                        ...
                        @else
                            <a href="{{ $clientesProceso->previousPageUrl() }}" class="px-3 py-1.5 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-900 dark:text-white rounded-md transition-colors border border-slate-200 dark:border-slate-700 text-xs">Anterior</a>
                        @endif
                        ...
                        <span class="px-3 py-1.5 bg-slate-100 dark:bg-slate-800/50 text-slate-600 dark:text-slate-400 rounded-md border border-slate-200 dark:border-slate-700 text-xs">
                            Página {{ $clientesProceso->currentPage() }} de {{ $clientesProceso->lastPage() }}
                        </span>
                        
                        @if($clientesProceso->hasMorePages())
                            <a href="{{ $clientesProceso->nextPageUrl() }}" class="px-3 py-1.5 bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-900 dark:text-white rounded-md transition-colors border border-slate-200 dark:border-slate-700 text-xs">Siguiente</a>
                        @else
