...
<body class="bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-white transition-colors duration-200 min-h-screen">
            ...
            <form id="tarea-form" class="p-3 md:p-4 space-y-3 overflow-y-auto max-h-[60vh]">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="text" name="texto" id="tarea_texto" required placeholder="Descripción de la tarea" class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-800 dark:text-white placeholder-slate-500 focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 focus:outline-none transition-all text-sm">
                    </div>
                    
                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">Lista</label>
                        <select name="lista_id" id="tarea_lista_id" class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-800 dark:text-white focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 focus:outline-none transition-all text-sm">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    <div>
                        ...
                        <input type="datetime-local" name="fecha_hora" id="tarea_fecha_hora" required class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg text-slate-800 dark:text-white focus:border-violet-500 focus:ring-2 focus:ring-violet-500/20 focus:outline-none transition-all text-sm">
                    </div>
                    
                    <div>
                        <label class="block text-xs font-medium text-slate-700 dark:text-slate-300 mb-1.5">Recurrencia</label>
                        <select 
                            name="recurrencia" 
                ...
                <div class="flex gap-2 pt-2">
                    <button type="submit" class="flex-1 px-4 py-2 rounded-lg bg-violet-500 hover:bg-violet-600 text-white text-sm font-medium transition-all">Guardar</button>
                    <button type="button" id="deleteTareaBtn" onclick="deleteTarea()" class="px-4 py-2 rounded-lg bg-red-500 hover:bg-red-600 text-white text-sm font-medium transition-all hidden">Eliminar</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        document.getElementById('tarea-form')?.addEventListener('submit', async function(e) {
            ...
            const notificacionTipo = notificacionHabilitada ? document.getElementById('tarea_notificacion_tipo').value : null;
            let notificacionMinutosAntes = null;
            let notificacionFechaHora = null;
            
            if (notificacionHabilitada) {
                if (notificacionTipo === 'relativa') {
                    notificacionMinutosAntes = parseInt(document.getElementById('tarea_notificacion_minutos_antes').value) || 60;
                } else if (notificacionTipo === 'especifica') {
                    notificacionFechaHora = document.getElementById('tarea_notificacion_fecha_hora').value;
                }
            }
