                                    ...
                                    @if($producto->foto_qr_super)
                                        @php
                                            ...
                                            if (str_starts_with($qrSuperPath, 'http://') || str_starts_with($qrSuperPath, 'https://')) {
                                                ...
                                                // Extraer solo el nombre del archivo y usar la ruta específica de storage
                        ...
                        <div class="space-y-4">
                            ...
                            <div>
                                ...
                                <input 
                                    ...
                                    id="productoDlc" 
                                    name="dlc" 
                                    value="{{ $producto->dlc ? \Carbon\Carbon::parse($producto->dlc)->format('Y-m-d') : '' }}"
                                    class="w-full px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                                >
                            </div>
                        </div>
        ...
        async function generateQRCode() {
            try {
                ...
                if (preview) {
                    ...
                    newPreview.className = 'mt-2 relative inline-block';
                    newPreview.innerHTML = `
                        <img src="${data.qr_code}" alt="Generated QR" class="w-32 h-32 object-cover rounded-lg border border-slate-300 dark:border-slate-600" id="currentQrPreview">
                        <button 
                            type="button"
                            onclick="removeFotoQr()"
                            class="absolute top-1 right-1 bg-red-500 hover:bg-red-600 text-white rounded-full p-1.5 shadow-lg transition-colors"
