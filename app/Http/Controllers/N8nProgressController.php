...
class N8nProgressController extends Controller
{
    ...
    public function update(Request $request)
    {
        try {
            ...
            // Si el workflow se completó o falló, verificar si hay workflows en cola para enviar
            if (in_array($request->status, ['completed', 'failed'])) {
                $this->processNextPendingWorkflow();
            }
     ...
     */
    private function processNextPendingWorkflow(): void
    {
        try {
            // Buscar el siguiente workflow en cola (pending) ordenado por fecha de creación
            $nextWorkflow = WorkflowRun::where('status', 'pending')
